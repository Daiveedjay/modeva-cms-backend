...
// SuspendAdmin godoc
// @Summary Suspend admin (Super admin only)
// @Description Suspend an admin account. Super admin only.
// @Tags Admin - Management
// @Accept json
// @Produce json
// @Security BearerAuth
// @Param id path string true "Admin ID to suspend"
// @Success 200 {object} models.ApiResponse
// @Failure 400 {object} models.ApiResponse "Cannot suspend yourself"
// @Failure 403 {object} models.ApiResponse "Super admin access required"
// @Failure 404 {object} models.ApiResponse "Admin not found"
// @Router /admin/admins/:id/suspend [post]
func SuspendAdmin(c *gin.Context) {
	...
	adminID := c.Param("id")
	log.Printf("[admin.suspend] request: %s by %s", adminID, adminIDStr)

	// Can't suspend yourself
	if adminID == adminIDStr {
		c.JSON(http.StatusBadRequest, models.ErrorResponse(c, "Cannot suspend your own account"))
		return
	}
	...
	if err := config.CmsGorm.WithContext(ctx).
		...
		if err == gorm.ErrRecordNotFound {
			...
			log.Printf("[admin.suspend] database error: %v", err)
	...
	if err := config.CmsGorm.WithContext(ctx).
		...
		log.Printf("[admin.suspend] failed to suspend: %v", err)
		c.JSON(http.StatusInternalServerError, models.ErrorResponse(c, "Server error"))
		return
	}
	...
	changes := map[string]interface{}{
		...
		"after":  map[string]interface{}{"status": "suspended"},
	}
	...
	activityLog := models.ActivityLog{
		...
		Action:       models.ActionSuspendAdmin,
	...
	if err := config.CmsGorm.WithContext(ctx).Create(&activityLog).Error; err != nil {
		log.Printf("[admin.suspend] failed to log activity: %v", err)
		// Don't fail the request, just log the error
	}

	log.Printf("[admin.suspend] success: %s suspended by %s", adminID, adminIDStr)
	c.JSON(http.StatusOK, models.SuccessResponse(c, "Admin suspended", nil))
}
